...
- **Engine RPM** → `reject_engine_rpm`. Signal: custom `rpm` key. Reject when RPM
  exceeds a configured idle threshold — the definitive engine-use reject that R16
  only approximates.
- **Engine state** → `reject_engine_on`. Signal: custom engine on/off key. Reject
...
- **Heel / roll** — infer overpowered/underpowered sailing state; could reject or
  ...
  `navigation.attitude`, or a custom plugin); AvNav core exposes no built-in
