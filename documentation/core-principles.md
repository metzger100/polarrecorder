...
1. **Stdlib-only runtime.** No pip dependencies in `plugin.py` or `server/polarrecorder/`. Users install by dropping a directory; no `pip install`.
...
8. **Single lock, no nesting.** Threading is `plugin.py`'s responsibility. `server/polarrecorder/` modules are lock-unaware and thread-unaware.
