...
These rules have the highest documentation precedence in the project.

## Key Details

1. **Stdlib-only runtime.** No pip dependencies in `plugin.py` or `polarrecorder/`. Users install by dropping a directory; no `pip install`.
2. **AvNav boundary isolation.** `polarrecorder/` never imports AvNav modules. Only `plugin.py` touches the AvNav API. The API is injected via protocols and fakes.
3. **No build step.** Runtime files are served as-is by AvNav.
...
8. **Single lock, no nesting.** Threading is `plugin.py`'s responsibility. `polarrecorder/` modules are lock-unaware and thread-unaware.
...
12. **File size limits are absolute.** A 400 non-empty-line hard limit applies to `plugin.py`, `polarrecorder/`, `tests/`, and root JS files; split modules instead of compressing code.
