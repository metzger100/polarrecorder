...
Polar Recorder resists poisoning by combining a validation pipeline with sparse per-bin histograms and percentile-based
reads. The model learns from accepted sailing samples only; rejected and quarantined samples are diagnostic signals, not
polar-speed inputs.
...
R16 quarantine, so motoring is excluded without discarding genuine light-air sailing. A depth signal rejects
shallow-water squat (R19).
...
is a Galilean translation of that frame: it changes SOG/COG but shifts the recorded TWS and the resulting STW
_together_, leaving the `(STW, TWS_water)` point valid. A current-_magnitude_ reject would discard good data, and as a
