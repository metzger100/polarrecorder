...
| R15  | `stability_window`            | reject     | `reject_warming_up`, `reject_unstable`                                    | Eligible history plus current sample must span `stability_window_seconds`, default 15 s, without a gap over 3 sample intervals; filled ranges stay below TWA 20 deg, TWS 10 kt, STW 4 kt defaults. | P-TWA/TWS/STW                 |
...
the active window. Missing, malformed, stale, out-of-range, non-sailing, motoring, shallow, and paused observations
clear that buffer. R15 also requires every retained gap to be at most three configured sample intervals. These rules
tolerate normal loop jitter without allowing sparse endpoints or pre-recovery data to prove a continuous stable period.
