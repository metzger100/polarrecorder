...
| R15  | `stability_window`            | reject     | `reject_warming_up`, `reject_unstable`                                    | Eligible history plus current sample must span the configured window, have at least `max(3, ceil(window / (sample interval × 1.10)) + 1)` observations, and no gap over 3 sample intervals. | P-TWA/TWS/STW                 |
