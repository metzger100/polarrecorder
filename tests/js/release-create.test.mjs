/**
 * Self-tests for tools/release-create.mjs. All git/npm/archive-builder
...
function makeFakeRoot() {
  ...
  for (const file of ["plugin.css", "plugin.js", "plugin.json", "plugin.mjs", "plugin.py"]) {
    fs.copyFileSync(path.join(process.cwd(), file), path.join(root, file));
  }
  fs.cpSync(path.join(process.cwd(), "viewer"), path.join(root, "viewer"), { recursive: true });
  fs.cpSync(path.join(process.cwd(), "server"), path.join(root, "server"), { recursive: true });
  return root;
}
...
test("aborts closed when the archive builder fails, before any commit/tag", () => {
  const root = makeFakeRoot();
  writeNotes(root, "1.0.0");
  const fake = fakeRunCommand();
  assert.throws(
...
test("happy path runs exactly one gate invocation and the exact command sequence", () => {
  ...
  const summary = createRelease({
    ...
    archiveBuilder: () => ({ filesIncluded: 9, totalSizeBytes: 12345 })
  });
