/**
 * Self-tests for tools/check-agents-pointer.mjs, the replacement for a retired
 * AGENTS.md/CLAUDE.md byte-sync contract: CLAUDE.md must stay a short, valid pointer to
 * AGENTS.md and the mandatory preflight files, never drift back into a duplicate copy, and
 * never point at a file that has gone missing.
 */
...
import { runAgentsPointerCheck } from "../../tools/check-agents-pointer.mjs";
...
function makeFakeRoot() {
  ...
  fs.writeFileSync(
    path.join(root, "documentation", "conventions", "coding-standards.md"),
    "# Coding standards\n"
  );
  fs.writeFileSync(
    path.join(root, "documentation", "conventions", "smell-prevention.md"),
    "# Smell prevention\n"
  );
  return root;
}
...
test("the real repo CLAUDE.md is a valid pointer", () => {
  const result = runAgentsPointerCheck({ root: ROOT, print: false });
...
test("passes for a clean fixture root", () => {
  ...
  const result = runAgentsPointerCheck({ root, print: false });
  assert.equal(result.ok, true, result.failures.join("\n"));
  cleanup(root);
});
...
test("fails when CLAUDE.md does not link to AGENTS.md", () => {
  ...
  fs.writeFileSync(
    ...
    REAL_CLAUDE.replace(/\[AGENTS\.md\]\(AGENTS\.md\)/g, "AGENTS.md")
  );
  const result = runAgentsPointerCheck({ root, print: false });
  assert.equal(result.ok, false);
