...
Starter: `npm run starter:create -- --output=/absolute/path --id=my-plugin --name="My Plugin"`; quality profiles use
`--level quality --profile viewer-only` or `--profile python-plus-viewer`, with dependency-free runtime and blocking
`check:all`. See `CONTRIBUTING.md` for the source-owned distribution and archive/peer-copy workflow.
