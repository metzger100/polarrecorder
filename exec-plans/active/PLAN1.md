...
**Model policy (default, written into the agent files):**

- Controller (`plan-controller`): `deepseek/deepseek-v4-pro`.
- Pro verifiers (`pro-requirements-verifier`, `pro-quality-verifier`): `deepseek/deepseek-v4-pro`.
- Worker subagents (`context-scout`, `implementation-worker`, `test-worker`, `docs-worker`): `deepseek/deepseek-v4-flash`.
...
- **Slug verification (one-time setup check):** the two slugs above (`deepseek/deepseek-v4-pro`, `deepseek/deepseek-v4-flash`) are the intended defaults. Before the first `/exec-plan` run, confirm both resolve in the live OpenRouter catalog (model lists change over time); if either has been renamed or retired, update it in the affected agent files and re-run. This is a configuration check, not a code change.
...
- The intended model policy is encoded: controller and Pro verifiers on `deepseek/deepseek-v4-pro`; workers on `deepseek/deepseek-v4-flash`.
