...
- Implementation begins with a **human-authored, inspection-verified foundation** spanning two phases. **Phase 0 (Kilo Agent Bootstrap)** creates the generic Kilo Code multi-agent setup (`.kilo/agents/`, `.kilo/commands/`); **Phase 1 (Repository Bootstrap)** creates the skeleton, the AI instructions (`AGENTS.md`/`CLAUDE.md`) and the quality gate (`tools/check-all.sh`) that the agents are themselves bound by. Neither implements product functionality. Because Phase 1 builds the very rulebook and gate the agents and Pro verifiers depend on — and would otherwise be self-verified by a gate it is simultaneously creating — it is **authored and verified by a human, not the agents, exactly like Phase 0**. The **agent-driven product build begins at Phase 2**, by which point a complete, trusted foundation and a working `check-all.sh` already exist, so the "deterministic gate is the binding arbiter" rule (§10 Phase 0, Verifier contract) holds uniformly for every agent-driven phase with no self-verification exception.
...
│
├── .kilo/                              # Kilo Code multi-agent config (Phase 0; version-controlled, generic across plans)
│   ├── README.md                       # Usage note: OpenRouter setup, /exec-plan, /verify-phase
│   ├── agents/
│   │   ├── plan-controller.md
│   │   ├── context-scout.md
...
│   │   ├── test-worker.md
│   │   ├── docs-worker.md
│   │   ├── pro-requirements-verifier.md
│   │   └── pro-quality-verifier.md
│   └── commands/
│       ├── exec-plan.md
│       └── verify-phase.md
│
...
### Phase 0 — Kilo Agent Bootstrap

**Intent:** Create the generic, project-optimized Kilo Code multi-agent setup that will drive the phase-by-phase implementation of this and any future active execution plan. **This phase creates agent/workflow infrastructure only — it implements no polarrecorder product functionality, no repository skeleton, no source, no tests, and no quality tooling.** It is a bootstrap phase: at the moment it is implemented, the repository contains only `exec-plans/active/PLAN1.md` and `misc/` (the AvNav and dyninstruments source zips). Phase 0 must therefore assume that `.kilo/`, project docs, `AGENTS.md`/`CLAUDE.md`, source files, package files, and quality scripts **do not yet exist**, and the agents it creates must degrade gracefully to the active plan until those artifacts appear in later phases.

**Dependencies:** None. (Phase 0 is the first phase; Phase 1 depends on it.)

**Deliverables:**

- `.kilo/agents/plan-controller.md` — primary orchestration agent (see behavioral spec below).
- `.kilo/agents/context-scout.md` — read-only repository/doc explorer subagent.
- `.kilo/agents/implementation-worker.md` — scoped implementation subagent.
- `.kilo/agents/test-worker.md` — testing / quality-check subagent.
- `.kilo/agents/docs-worker.md` — documentation subagent.
- `.kilo/agents/pro-requirements-verifier.md` — strict read-only requirements verifier subagent.
- `.kilo/agents/pro-quality-verifier.md` — strict read-only quality/architecture verifier subagent.
- `.kilo/commands/exec-plan.md` — slash command to start/continue implementing the active plan.
- `.kilo/commands/verify-phase.md` — slash command to run independent verification only.
- A short **usage note** (in `.kilo/README.md` or at the top of `exec-plan.md`) explaining how to: configure OpenRouter as the model provider in Kilo Code, run `/exec-plan` to implement the next incomplete phase (the first phase not marked `done` in the active plan's `<PLAN>.progress.md` ledger), and run `/verify-phase` for verification only.

**Model policy (default, written into the agent files):** <!-- AMENDED A1: Kilo model IDs are provider-qualified OpenRouter IDs. -->

- Controller (`plan-controller`): `openrouter/deepseek/deepseek-v4-pro`.
- Pro verifiers (`pro-requirements-verifier`, `pro-quality-verifier`): `openrouter/deepseek/deepseek-v4-pro`.
- Worker subagents (`context-scout`, `implementation-worker`, `test-worker`, `docs-worker`): `openrouter/deepseek/deepseek-v4-flash`.
- **Token-efficiency rule** (stated in every agent file as applicable): Pro models are used **only** for planning, orchestration, and strict verification; Flash models do all repo exploration, implementation, tests, and docs. The controller delegates small, scoped tasks and **must not** dump the whole plan into worker prompts — it passes only the relevant phase intent, the relevant acceptance criteria, the relevant file paths, the relevant constraints, and the exact requested output format. **Scoping down must not strip interdependencies:** when a task slice is governed by a cross-cutting invariant (e.g. the candidacy/quality-gate split spanning §6.B/§6.C/§6.C2/§6.E, the lock/snapshot discipline in §7, the clock-injection rule, the dependency-direction graph), the controller includes that invariant verbatim (with its §-reference) in the worker prompt, because a Flash worker handed only the local slice can produce code that is locally plausible but violates a global contract. To stop that from passing silently, **worker-authored tests do not self-certify the work**: `pro-requirements-verifier` checks the implementation against the **named §11 acceptance criteria and the concrete worked examples** (e.g. P65 of `{58:12,59:47,60:31,61:18}` = 6.0 kt, the `total_seen` invariant, the round-trip equality test), not merely that the worker's own tests are green — and the deterministic `tools/check-all.sh` gate is the final arbiter regardless of which model wrote the code.
- **Slug verification (one-time setup check):** the two slugs above (`openrouter/deepseek/deepseek-v4-pro`, `openrouter/deepseek/deepseek-v4-flash`) are the intended defaults. Before the first `/exec-plan` run, confirm both resolve in the live OpenRouter catalog (model lists change over time); if either has been renamed or retired, update it in the affected agent files and re-run. This is a configuration check, not a code change.
- **Kilo capability verification (one-time setup check):** this agent design assumes the Kilo Code version in use actually supports (1) **per-agent model assignment** (controller/verifiers on Pro, workers on Flash), (2) **read-only subagents** (`context-scout` and both Pro verifiers must be unable to edit files or run destructive commands), and (3) the **delegation/nesting model** described here (the controller delegates to the named subagents; workers do not spawn further agents unless explicitly permitted). Before the first `/exec-plan` run, confirm Kilo's agent schema supports these; if a capability is missing or named differently, adapt the agent files to the closest supported equivalent (e.g. enforce read-only via tool-allowlist, or collapse roles) and note the deviation in `.kilo/README.md`. If read-only enforcement for the Pro verifiers is not available at all, the controller must treat their output as advisory and rely on the deterministic `tools/check-all.sh` gate as the binding arbiter. This is a configuration check, not product code.

**Generic-but-project-optimized requirements (stated in the agent files):** The agents are optimized for this project's workflow but are **not** hardcoded to PLAN1. They operate on whatever plan is active: they read `exec-plans/active/*.md` only as needed; once documentation exists they use `documentation/TABLEOFCONTENTS.md` and select **1–3 relevant files per task** rather than reading all docs; once `AGENTS.md`/`CLAUDE.md` exist they obey them (including the mandatory session preflight, §8); before docs exist they fall back to the active plan and visible repository files; and they never assume product details that are not present in the active plan or project docs.

**Phase-completion ledger (the single source of truth for "next incomplete phase"):** Because Kilo sessions are not durable, the controller cannot infer which phases are done from conversation state, and it must **not** guess from repository contents. Completion is tracked in a **per-plan progress ledger** that lives beside the active plan: for an active plan `exec-plans/active/<PLAN>.md` (e.g. `exec-plans/active/PLAN1.md`) the ledger is `exec-plans/active/<PLAN>.progress.md` (e.g. `exec-plans/active/PLAN1.progress.md`). It is version-controlled (the `.gitignore` does **not** ignore `exec-plans/`) and travels with the plan when the plan moves to `exec-plans/completed/`. Contract:

- **Format:** one line per phase, `Phase <N> — <pending|in-progress|done> — <ISO-date> — <short note>` (a Markdown table is equally acceptable). The phase list is derived from the active plan's Implementation Order, so the ledger is generic, not PLAN1-specific.
- **Initialization:** the ledger is seeded when the human completes the foundation. After authoring and inspection-verifying Phase 0 and Phase 1, the human creates `<PLAN>.progress.md` enumerating all of the active plan's phases with **Phase 0 and Phase 1 marked `done`** ("human bootstrap, inspection-verified") and **Phase 2 onward `pending`**. (If the ledger is somehow absent when `/exec-plan` first runs, the controller creates it with all phases `pending`, then refuses to proceed until the foundation precondition below is satisfied — it never implements Phase 0 or Phase 1 itself.)
- **Writer:** the `plan-controller` is the **only** writer. It sets a phase to `in-progress` when it begins work on it, and to `done` **only after both Pro verifiers return PASS** (never from worker summaries). A hard-blocked phase stays `in-progress` with the blocker noted.
- **Reader:** `/exec-plan` and `plan-controller` read the ledger first; the "next incomplete phase" is the first phase not marked `done`. This makes resume deterministic across fresh sessions.

**Plan-defect handling protocol & amendments ledger (how the controller resolves gaps, contradictions, and errors *in the plan itself*):** The active plan is the single source of truth (§1) and the Pro verifiers FAIL any *undocumented* deviation from it — so when implementation surfaces a defect in **the plan** (as opposed to a defect in the code), the controller must neither blindly implement a broken instruction nor let a worker quietly improvise around it. These defects are resolved by one fixed protocol, owned **exclusively by the `plan-controller`**, and logged in a **per-plan amendments ledger** `exec-plans/active/<PLAN>.amendments.md` — a companion to the progress ledger with the same rules (controller is the sole writer; version-controlled, since `.gitignore` does not ignore `exec-plans/`; travels with the plan into `exec-plans/completed/`). A "defect" is a **gap** (something required but unspecified), a **contradiction** (two sections that cannot both hold), or an **error** (a claim shown false — e.g. a mis-verified source fact). The protocol (run whenever the controller, a worker, or a Pro verifier flags a suspected plan defect):

1. **Confirm.** Delegate to `context-scout` (read-only) to confirm it is a genuine plan defect — verify error-class defects against the read-only source trees in `misc/`, and contradiction-class defects against the cited plan sections — not a misreading. A false alarm is closed with a one-line ledger note and work resumes unchanged.
2. **Classify.** **(a) Mechanical** — the correct resolution is *uniquely determined* by the rest of the plan (an obvious omission, a typo, an off-by-one citation, a fix already implied elsewhere). **(b) Judgment call** — more than one defensible resolution exists, OR the defect touches a Hard Constraint (§9), a core principle, the module-boundary / dependency-direction rules (§7), the persistence schema or API contract (§6.E / §6.F), the MVP scope (§4), or any externally-driven policy (e.g. licensing, specific threshold values).
3. **Resolve.** For **(a)**, adopt the uniquely-implied fix and proceed. For **(b)**, **stop and escalate to the human** with the defect, the options, and a recommended fix — never silently pick a judgment-call resolution, never silently broaden scope, never silently weaken a quality gate or a constraint. A phase with an open judgment-call defect stays `in-progress` (never `done`).
4. **Record.** Append one amendments-ledger entry per resolution (and per confirmed false alarm): `A<N> — <ISO-date> — Phase <P> — <gap|contradiction|error> — §<sections> — <mechanical|judgment> — <resolution> — <human decision (judgment calls only)>`.
5. **Reconcile the plan text.** Apply the *minimal* corrective edit to the affected plan section(s), tagged with an inline marker referencing the amendment (`<!-- AMENDED A<N>: <one-line> -->`), so the plan stays the authoritative spec and never contradicts the as-built code. The ledger is the audit trail; the edited section is the spec. (A defect large enough to reshape multiple phases is escalated instead, for a decision to author a subsequent plan, per §1.)

**Verifier contract:** a deviation from the plan's *original* wording is acceptable to both Pro verifiers **iff** a matching `A<N>` ledger entry exists (with a recorded human decision for any judgment-call entry) **and** the affected plan section carries the corresponding `AMENDED` marker. An unrecorded deviation, a judgment-call amendment with no human decision, or a plan edit with no ledger entry is a FAIL. This is the precise meaning of "no undocumented deviations from the plan" in the verifier specs below.

**Independent re-classification of amendments (guards against the controller mislabelling its own escalations):** the controller classifies each defect mechanical-vs-judgment, but it must **not** be the sole judge of that call. `pro-requirements-verifier` **independently re-classifies every `A<N>` entry** against the judgment-call triggers in step 2 of the protocol (touches a Hard Constraint §9, a core principle, the module-boundary/dependency-direction rules §7, the persistence schema or API contract §6.E/§6.F, the MVP scope §4, any external policy, or admits more than one defensible fix). A defect the controller labelled **mechanical** that matches any trigger — or that the verifier judges to have a non-unique resolution — is a **FAIL**: it should have been escalated to the human, and the phase cannot be marked `done` until it is. This makes the mechanical/judgment split itself reviewable rather than self-certified.

**Evidence-grounded PASS (guards against same-family rubber-stamping):** because the controller and both Pro verifiers may run on the same model family, semantic agreement between them is **not** evidence of correctness — shared blind spots are real. Every PASS must therefore be grounded in **objective artifacts**: the deterministic `tools/check-all.sh` output (the true backstop), the actual file diffs, and the **named** §11 acceptance criteria for the phase (each cited by the verifier). A verifier must **never** PASS by restating the controller's reasoning or a worker's summary; if the objective artifacts are unavailable or unrun, the verdict is FAIL, not "assumed PASS". (Configuration recommendation, not a hard requirement: assigning at least one verifier — e.g. `pro-requirements-verifier` — to a *different* model family than the controller further reduces shared blind spots. This is a model-policy knob set in the agent file, left to the operator; the evidence-grounded rule above is the in-band mitigation that holds regardless.)

**Required agent behavioral specifications (each `.kilo/agents/*.md` must encode at least the following):**

- **`plan-controller`** — Model `openrouter/deepseek/deepseek-v4-pro`; **primary** agent. Locate the active plan in `exec-plans/active/`; read the per-plan progress ledger (`<PLAN>.progress.md`, creating it from the plan's phase list if absent) and take the **first phase not marked `done`** as the current phase, along with its exit conditions; **never implement the human-authored foundation phases (Phase 0 and Phase 1)** — before starting its first product phase the controller confirms the foundation is in place (Phase 0 and Phase 1 marked `done` in the ledger, `AGENTS.md`/`CLAUDE.md` present, and `tools/check-all.sh` exits 0); if any is missing it **stops and asks the human to complete the foundation** rather than building it itself; convert that phase into a concise working checklist; ask the user only when the active plan or current phase cannot be identified. Delegate exploration to `context-scout`, code changes to `implementation-worker`, tests/checks to `test-worker`, and documentation to `docs-worker`. Mark the phase `in-progress` in the ledger when work begins. Request independent verification from **both** `pro-requirements-verifier` and `pro-quality-verifier`, and iterate fix→verify until both return PASS **or** the iteration bound is hit: after **3** full fix→verify cycles on one phase without both verifiers reaching PASS (or if two successive cycles surface the *same* unresolved failure), stop, leave the phase `in-progress`, record the blocker in `<PLAN>.progress.md` (and, if the blocker is a plan defect, in `<PLAN>.amendments.md`), and **escalate to the human** rather than thrashing. This bound is what makes "a hard blocker is documented" a concrete, reachable stop condition instead of an open-ended loop. When implementation surfaces a defect in the **plan itself** (a gap, contradiction, or error — not a code defect), apply the **plan-defect handling protocol** above (confirm via `context-scout` → classify mechanical vs. judgment-call → for mechanical, auto-fix and record; for judgment calls, stop and escalate to the human → reconcile the plan text with an `AMENDED` marker), logging every resolution in `<PLAN>.amendments.md`; never silently implement a broken instruction and never let a worker improvise around one. Never skip phases. Never mark a phase complete from worker summaries alone — inspect the actual changed files and diffs, and run or request the relevant checks; record the phase as `done` in the ledger **only** after both Pro verifiers PASS. Keep subagent prompts small and focused; avoid sending the full PLAN1.md unless absolutely necessary, sending only current phase intent, relevant checklist items, relevant constraints, relevant files, and the exact requested output format.
- **`context-scout`** — Model `openrouter/deepseek/deepseek-v4-flash`; **subagent**, read-only. Read only what is needed; identify relevant files, docs, commands, and constraints; prefer `documentation/TABLEOFCONTENTS.md` when docs exist, otherwise rely on the active plan and visible repo files. Does not edit; runs no destructive commands. Returns concise findings: relevant files, relevant docs, relevant constraints, likely implementation approach, and unknowns.
- **`implementation-worker`** — Model `openrouter/deepseek/deepseek-v4-flash`; **subagent**. Implement only the delegated task; do not broaden scope or perform unrelated refactors; preserve architecture and coding standards; follow the active plan and project docs; keep diffs minimal. Reports changed files, decisions, checks run, and limitations.
- **`test-worker`** — Model `openrouter/deepseek/deepseek-v4-flash`; **subagent**. Identify existing test patterns; add/update targeted tests for the delegated acceptance criteria; run relevant checks when safe, and when they cannot run, explain why and give the exact commands; do not invent unrelated test infrastructure. Reports test strategy, files changed, commands run, results, and failures.
- **`docs-worker`** — Model `openrouter/deepseek/deepseek-v4-flash`; **subagent**. Update only documentation relevant to the delegated task; maintain the project documentation format; keep docs concise and navigable; update documentation indexes when required; do not duplicate large plan sections into docs unless explicitly required. Reports changed files and consistency checks.
- **`pro-requirements-verifier`** — Model `openrouter/deepseek/deepseek-v4-pro`; **subagent**, read-only (does not edit). Compare repository state to the current phase's requirements and the PLAN acceptance criteria, verifying actual files and diffs (not summaries). Return PASS only if the phase requirements are satisfied; on FAIL, list the exact missing requirements and follow-up tasks. Focus: functional correctness, acceptance criteria, phase deliverables, no skipped requirements, no undocumented deviations from the plan — where a deviation counts as *documented* **only** if it has a matching `A<N>` entry in `<PLAN>.amendments.md` (with a recorded human decision for any judgment-call entry) **and** the affected plan section carries the corresponding `AMENDED` marker; an unrecorded deviation, a judgment-call amendment lacking a human decision, or a plan edit with no ledger entry is a FAIL. Also verify ledger ↔ plan-text ↔ code consistency for every amendment; **independently re-classify** each amendment (a "mechanical" label that matches a judgment-call trigger, or whose fix is not uniquely determined, is a FAIL — see "Independent re-classification" in the Verifier contract); and **ground every PASS in objective artifacts** (the `tools/check-all.sh` result, actual diffs, and the named §11 acceptance criteria), never in agreement with the controller's or a worker's narrative.
- **`pro-quality-verifier`** — Model `openrouter/deepseek/deepseek-v4-pro`; **subagent**, read-only (does not edit). Verify project constraints, coding standards, docs rules, test quality, file-size limits, smell-prevention rules, quality gates, and preservation of token-efficient documentation practices. Return PASS only if implementation quality is acceptable; on FAIL, list the exact quality violations and follow-up tasks. Focus: architecture boundaries, maintainability, tests, docs consistency, no overbroad changes, no hidden complexity, no generated bloat, no secrets, no destructive or unnecessary tooling, and — for any plan amendment — that the corrective plan edit is minimal and scoped, with no quality gate, Hard Constraint, or core principle weakened except via a judgment-call amendment carrying an explicit recorded human decision. Ground every PASS in objective artifacts (the `tools/check-all.sh` result and actual diffs), not in agreement with the controller — same-family agreement is not evidence (see "Evidence-grounded PASS" in the Verifier contract).

**Required slash-command specifications:**

- **`.kilo/commands/exec-plan.md`** — Starts or continues implementing the active execution plan. Uses `plan-controller`; locates the active plan in `exec-plans/active/`; determines the next incomplete phase **by reading the per-plan progress ledger `<PLAN>.progress.md`** (the first phase not marked `done`; creates the ledger from the plan's phase list on first run); verifies the human-authored foundation is complete before driving any phase (Phase 0 and Phase 1 marked `done` and `tools/check-all.sh` green) and never implements those foundation phases itself; executes **only** that phase unless the user explicitly asks for more; when a plan defect is surfaced during the phase, runs the plan-defect handling protocol (auto-fixing mechanical defects into `<PLAN>.amendments.md` and the plan text, escalating judgment calls to the human and pausing the phase as `in-progress`); requires **both** Pro verifiers to PASS before the phase is recorded `done` in the ledger; produces a final phase report.
- **`.kilo/commands/verify-phase.md`** — Runs independent verification on the current or a named phase. Uses both Pro verifier agents; verifies actual repository state against the active phase; edits no files; returns each verifier's PASS/FAIL plus required follow-up tasks.

**Constraints:**

- Phase 0 may be implemented before any project code exists; it creates **only** agent/workflow infrastructure under `.kilo/`.
- Phase 0 must **not** implement polarrecorder product functionality, repository skeleton, source, tests, or quality tooling (those are Phase 1+).
- Phase 0 must **not** modify AvNav or dyninstruments source. It must **not** unpack the reference zips in `misc/` unless genuinely needed to write project-optimized instructions; if unpacking is needed it must be **read-only** and kept under `misc/` or a clearly git-ignored temporary location.
- Phase 0 must **not** commit API keys or model-provider secrets (the model strings above are not secrets; the OpenRouter key is configured in Kilo Code, never in the repo).
- `.kilo/agents/` and `.kilo/commands/` are version-controlled project configuration.
- Agents must remain **generic** for the project and the active-plan workflow (not PLAN1-specific).
- Worker subagents must not spawn further agents unless explicitly allowed by Kilo's schema and actually needed; the two Pro verifier agents are strictly read-only.

**Exit conditions:**

- All required `.kilo/agents/*.md` files exist (`plan-controller`, `context-scout`, `implementation-worker`, `test-worker`, `docs-worker`, `pro-requirements-verifier`, `pro-quality-verifier`).
- All required `.kilo/commands/*.md` files exist (`exec-plan`, `verify-phase`).
- Agent prompts are generic (operate on the active execution plan) and do **not** hardcode PLAN1-specific product tasks.
- Agent prompts reference the active-execution-plan workflow and include the token-efficiency rules above.
- The progress-ledger mechanism is encoded: `plan-controller`/`exec-plan` determine the next phase from `<PLAN>.progress.md` (first phase not marked `done`), the controller is the sole writer, and a phase is recorded `done` only after both Pro verifiers PASS.
- The **plan-defect handling protocol** is encoded in `plan-controller` and the two Pro verifier agents: confirm → classify (mechanical vs. judgment call) → auto-fix-and-record for mechanical, stop-and-escalate-to-human for judgment calls → reconcile the plan text with an `AMENDED` marker; the controller is the sole writer of `<PLAN>.amendments.md`; and "documented deviation" is defined for the verifiers as a matching ledger entry (plus a human decision for judgment calls) with the corresponding plan-text `AMENDED` marker.
- The verification loop is bounded and reviewable: the controller stops after **3** fix→verify cycles (or a repeated identical failure), escalating to the human rather than thrashing; `pro-requirements-verifier` **independently re-classifies** every amendment (mislabelled mechanical → FAIL); and both Pro verifiers **ground PASS in objective artifacts** (`tools/check-all.sh`, diffs, named §11 criteria), never in agreement with the controller.
- The one-time setup checks are documented in the usage note: OpenRouter slug verification **and** Kilo capability verification (per-agent models, read-only subagents, delegation/nesting); if read-only enforcement is unavailable, the deterministic gate is the binding arbiter and verifier output is advisory.
- The intended model policy is encoded: controller and Pro verifiers on `openrouter/deepseek/deepseek-v4-pro`; workers on `openrouter/deepseek/deepseek-v4-flash`.
- `plan-controller` is permitted to delegate only to the intended subagents; the Pro verifier agents are read-only; worker agents do not spawn further agents unless explicitly allowed and needed.
- A short usage note explains: configure OpenRouter in Kilo Code, use `/exec-plan` to implement the next phase, use `/verify-phase` for verification only.
- No product source code has been implemented in Phase 0; no secrets or API keys are present anywhere in the repository.

**Verification requirements:** Because no `tools/check-all.sh` exists yet, Phase 0 is verified by inspection rather than the automated gate: confirm each file above exists with the specified model, mode (primary vs. subagent), delegation/read-only permissions, and token-efficiency rules; confirm no `.kilo` file hardcodes PLAN1 product details; and confirm no secrets are committed. Once Phase 1 has produced the tooling, `/verify-phase` (the two Pro verifiers) can be re-run against Phase 0's `.kilo/` artifacts as a regression check.
...
- Top-level files at the repository root (the `polarrecorder/` root already exists — it holds `exec-plans/active/PLAN1.md`, `misc/`, and the `.kilo/` config created in Phase 0; this phase **adds** the files, it does not create the root): `AGENTS.md`, `CLAUDE.md`, `README.md`, `CONTRIBUTING.md`, `ARCHITECTURE.md`, `ROADMAP.md`, `CHANGELOG.md`, `pyproject.toml`, `package.json`, `.gitignore`.
...
- **Human records Phase 0 and Phase 1 as `done`** in `<PLAN>.progress.md` ("human bootstrap, inspection-verified"). Only then does the agent-driven loop have a complete, trusted foundation; `/exec-plan` refuses to start until this is recorded and the gate is green (§10 Phase 0), and the controller's first phase is Phase 2.
...
- [ ] Phase 0 creates project-local Kilo agent definitions (all seven `.kilo/agents/*.md`).
- [ ] Phase 0 creates project-local Kilo workflow commands (`.kilo/commands/exec-plan.md`, `.kilo/commands/verify-phase.md`).
- [ ] Agents are generic for active execution plans, not hardcoded to PLAN1.
- [ ] Agents enforce token-efficient context use (Pro for orchestration/verification, Flash for workers; controller delegates small scoped tasks and passes only the relevant phase/criteria/files/constraints).
- [ ] The `plan-controller` requires PASS from **both** Pro verifiers before marking a phase complete, and verifies actual files/diffs rather than worker summaries.
- [ ] The `plan-controller` and Pro verifiers encode the plan-defect handling protocol: defects in the plan itself are confirmed, classified (mechanical → auto-fix-and-record; judgment call → escalate to the human), recorded in `<PLAN>.amendments.md`, and reconciled into the plan text with an `AMENDED` marker; the verifiers treat only ledger-recorded, marker-tagged changes (human-decided where judgment is involved) as documented deviations.
...
| Document | Created In |
|---|---|
| `.kilo/agents/plan-controller.md` | Phase 0 |
| `.kilo/agents/context-scout.md` | Phase 0 |
| `.kilo/agents/implementation-worker.md` | Phase 0 |
| `.kilo/agents/test-worker.md` | Phase 0 |
| `.kilo/agents/docs-worker.md` | Phase 0 |
| `.kilo/agents/pro-requirements-verifier.md` | Phase 0 |
| `.kilo/agents/pro-quality-verifier.md` | Phase 0 |
| `.kilo/commands/exec-plan.md` | Phase 0 |
| `.kilo/commands/verify-phase.md` | Phase 0 |
| `AGENTS.md` | Phase 1 |
| `CLAUDE.md` | Phase 1 |
