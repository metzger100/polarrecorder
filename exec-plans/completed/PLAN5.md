...
consistently used script name is `check:docs`, and every such reference was run/treated as
that substitution rather than adding a duplicate alias; (2) Phase 10E's manual runtime
smoke was performed at the mock-server HTTP/API level (every viewer/API endpoint reachable
with correctly shaped JSON) rather than full interactive-browser visual confirmation, since
this execution environment has no display or browser -- consistent with the plan's own "No
browser automation is required" allowance, recorded honestly rather than claimed as a full
visual pass. No commit, tag, push, release artifact, or GitHub Release was created as part
of this migration.
...
- **10A (clean, offline-capable setup):** A full working-tree copy (including `.git`, since
  ...
  `node_modules`/`venv`/`coverage`/`.hypothesis` excluded. `npm run setup` ran clean from
  that state and produced a fresh `node_modules` and `venv`. Network was then intercepted
  (`http_proxy`/`https_proxy`/`HTTP_PROXY`/`HTTPS_PROXY` pointed at an unreachable
  `127.0.0.1:1`, `npm_config_offline=true`) and `npm run actions:lint`, `npm run
check:standard`, and `npm run check:core` all still passed, proving ordinary checks use
  only locked/cache-local inputs. The disposable copy was deleted afterward.
- **10B (complete focused/aggregate proof):** Every listed command passed: `format:check`,
  `lint`, `actions:lint`, `duplication:check`, `typecheck`, `test:split` (387 pytest + full
  Node suite), `test:contract`, `test:focus:check`, `check:python-contracts`,
  `package:check`, `check:smells`, `check:complexity` (0 tracked baseline entries, 0
  regressions), `check:scaling`, `check:docs` (substituting for the plan's
  never-introduced `docs:check`, see Status), `check:filesize`, `test:coverage:check`
  (Python 95.77% aggregate; viewer/plugin JS c8 floors all green;
  `check:coverage-inventory` clean), `check:all`, `tools/check-all.sh` (confirmed it only
  runs `npm run check:all`, no divergent second graph), and `git diff --check`.
  Final counts: 387 pytest tests passed; 225 `test:tools` Node tests passed (full
  `test:node`/`test:split` suite substantially larger); complexity baseline 0 entries (all
  4 Phase 0 findings already resolved in Phases 2/4); release manifest 58 runtime
  file/directory entries.
- **10C (clone-local hook behavior):** `npm run hooks:doctor` and `npm run hooks:install`
  both pass in the real implementation clone (`core.hooksPath` was already `.githooks`
  from earlier setup); the isolated-unconfigured-temp-repo fail-then-repair behavior is
  covered by `tests/js/hooks.test.mjs` (12 tests, part of the passing `test:tools` run).
- **10D (negative residue checks):** Confirmed exactly one `.github/workflows/*.yml`
  file; no `CODEOWNERS`, `.pre-commit-config.yaml`, mutation-testing config/dependency, or
  browser-driver dependency anywhere; no `check-performance.py`; no `check:ci` script; no
  branch/PR trigger (workflow trigger is `push: tags: ["v*"]` only); both workflow `uses:`
  pinned to full 40-character commit SHAs (no mutable tag); no ordinary-gate `curl`/`wget`
  outside `install.sh`/`setup`/`actionlint.sh`; `git status --porcelain --ignored` shows
  `venv/`, `node_modules/`, `.hypothesis/`, and `coverage/` all correctly ignored (`!!`),
  never untracked (`??`); no devDependency uses a version range; the smell-catalog and
  rule-parity-ledger completeness checks (part of `check:docs`/pytest) prove no generic
  checker is retained without a parity-ledger reason.
- **10E (manual runtime smoke):** Started `tools/mock-server.py` and confirmed
  `viewer/viewer.html`, `viewer/viewer.js`, and `viewer/viewer.css` all serve `200`, and
  `/api/status`/`/api/timeline` return correctly shaped JSON (`current_values`,
  `current_decision`, `counters`, timeline `buckets` with `accepted`/`rejected`/
  `quarantined`/`reasons`). Full interactive-browser visual confirmation (status cards,
  history strip, tooltip, day/night polar chip colors, presets, timeline chart, export
  preview, settings tab) was not performed, since this execution environment has no
  display or browser -- an explicitly recorded limitation, not a claimed pass; the
  underlying rendering logic is otherwise covered by `tests/js/viewer-*.test.mjs`'s
  fake-DOM suite, which passed as part of `test:node`.
- **Discovered and fixed during 10F closeout:** `package-lock.json` existed on disk but
  was untracked (`git status` showed `?? package-lock.json`) even though `package.json`
  itself has been tracked since before this migration -- a pre-existing gap against the
  "committed lockfile" acceptance criterion. Staged it with `git add` (staging only, no
  commit, per the standing no-commit-without-authorization constraint).
...
- [x] `package.json` is private, uses non-release version `0.0.0-test`, declares Node 26/npm 12.0.1, has exact direct
      dev pins, and has a committed lockfile.
- [x] Python developer dependencies are exact/hash-locked and never become runtime dependencies.
- [x] A Phase 0-verified machine-readable contract freezes the supported developer-Python range, preferred interpreter,
...
- [x] `requirements-dev.in` and `requirements-dev.txt` have one tested maintainer-only `requirements:lock` generation
      path, and setup installs the resolved lock with `--require-hashes`.
- [x] `npm run setup` installs locked Node/Python tools and provisions pinned actionlint; ordinary gates perform no
      downloads.
- [x] `check:all` expands exactly to `check:core` plus `test:coverage:check`; `check:strict` aliases it and `check:ci`
      is absent.
- [x] `tools/check-all.sh` is only a compatible repository-rooted wrapper around the canonical npm aggregate.
- [x] Phase 1 keeps the old complete gate authoritative while adoption debt exists; Phase 2 activates the now-clean
...
- [x] The temporary `check:migration` aggregate keeps all old and newly active protections blocking through Phases 2-7,
      is recursion-tested and undocumented outside the plan, and is absent after Phase 8 promotes the final graph.
- [x] `check:standard`, `check:fast`, `check:core`, `test:split`, and focused commands have documented, tested meanings.
- [x] The literal `check:core` graph reaches Ruff/mypy, every retained Python contract, pytest, and every Node
      tool/contract/viewer/plugin suite exactly once; `test:focus:check` is a direct required group.
- [x] `duplication:js` owns jscpd plus the retained structural JavaScript owner unless direct parity proves replacement;
...
- [x] Phase 0 canonical test/coverage/complexity-source captures predate active ledgers, exclude volatile/raw-report
...
- [x] The temporary strict test owner is active before Phase 0 adds its first executable JavaScript proof and remains
      active until Phase 5 replaces it atomically.
- [x] Setup, Hypothesis, and c8 use ignored owned paths and a clean-state test proves they leave no stray generated
      repository state.

### Maintained tools and parity

- [x] `format` and `format:check` share one exact Prettier/Ruff inventory covering package/lock metadata, workflows,
...
- [x] A machine-readable complete maintained-file disposition gives every unsupported format, including probed TOML,
...
- [x] ESLint, Stylelint, markdownlint, Linkinator, actionlint, and jscpd pass with deliberate negative fixtures.
- [x] `--polarrecorder-*` CSS namespace enforcement passes and visual behavior is unchanged.
- [x] Every old checker/rule has a final owner; no custom checker was deleted before positive/negative parity proof.
- [x] Retained custom tools own only Polar-specific contracts and have focused self-tests.
- [x] Python architecture, compatibility, finite-value, suppression, and duplicate-helper protections remain blocking.
- [x] jscpd has an exact maintained scan-scope contract, and renamed-function/normalized-block JavaScript clones remain
      blocking through a retained focused owner unless direct parity proves jscpd catches both semantics.
- [x] `duplication:js`, `duplication:python`, and `duplication:check` reach every underlying clone owner exactly once,
      including the retained structural JavaScript owner when required.
- [x] Phase 0 captures Ruff lint/format debt for every maintained Python tool with the selected locked version; Phase 2
...
- [x] Structured boundary markers validate category/owner/date/reason/expiry and fail when malformed, stale, misplaced,
      unused, or applied to another rule.
- [x] Generic production suppressions are blocked; direct/computed/composed HTML and `on*` sinks fail outside an exact
      reviewed owner (initially none).
- [x] Selected near-limit files have immutable explicit hotspot budgets below 400, with missing/over-budget negative
      proof.

### Typing, tests, coverage, and complexity

- [x] Every plugin/viewer source file is strict no-emit `checkJs` input with no `@ts-ignore`, `@ts-nocheck`, broad
      `any`, or runtime build output.
- [x] `viewer/status-ui.js` and every other source file created before Phase 4 are strict, carry at least the current
      80-percent line-coverage default, and satisfy 10/40/4/6 complexity limits from their first commit.
- [x] Every JavaScript function added to a pre-existing shipped source file before Phase 7 also receives 10/40/4/6
...
- [x] Every executable JavaScript test/helper, including the viewer harness, is inventory-owned and strict; the immutable
      executable exception baseline is empty.
- [x] Every executable JavaScript test/helper added in Phases 0-4 is strict from its first phase through the temporary
      post-capture discovery typecheck, which Phase 5 replaces atomically with the complete strict test inventory.
- [x] Every committed negative quality fixture is non-executable, lives at an exact preauthorized
...
- [x] JavaScript and Python focused/disabled tests are blocked; the accepted exception set is empty unless this plan is
      amended with evidence.
- [x] All shipped Python/browser files are measured or wholly contract-owned; `plugin.py` is measured with reviewed
      per-file line/branch floors, and missing/stale/lowered/self-grandfathered coverage entries fail.
- [x] Every contract-owned coverage owner is a normalized repository-relative, runner-discovered executable test in
...
- [x] Aggregate 90 percent, validation 95/95, histogram 95/90, and every old per-viewer floor are preserved or raised.
- [x] Native c8 global and viewer/entrypoint family thresholds enforce at least 80 percent lines/functions/statements
      and 65 percent branches, with negative proof for every metric.
- [x] New behavioral files default to at least 80 percent line and 65 percent branch coverage.
- [x] JavaScript complexity uses immutable Polar provenance and an active no-regression/shrink-only ledger; Python Ruff
      limits are unchanged.
- [x] Hypothesis invariants and deterministic model/projection/full-`api_handlers.format_polar` scaling contracts pass
      with correctness assertions; raw-bin and grid-cell growth are both bounded and a curve-assembly regression fails.
- [x] No wall-clock performance, mutation, or browser-automation gate remains.

### File size and runtime preservation

- [x] `viewer/viewer.js` is split before formatter/type migration and every resulting runtime file stays below 400
      non-empty lines.
- [x] README and all other covered source/test/docs remain below 400 without one-line compression.
- [x] Viewer module order/dependency metadata, namespace, API behavior, status rendering, actions, CSS colors, and
      legacy/modern entrypoints remain green.
- [x] Python runtime, AvNav boundary, locking, persistence, configuration, validation, API, export/import, and package
      contents are unchanged.
- [x] Historical release ZIPs/notes and completed plans remain untouched.

### Hooks, release, and GitHub

- [x] The executable pre-push hook runs exactly one root-scoped `npm run check:all` and propagates failure.
- [x] Hook install/doctor are explicit, idempotent, tested in temp repositories, and never hidden in setup/lifecycle
      scripts.
- [x] Release prepare has clean-tree/help/argument contracts; release create allows only canonical notes dirt and gates
      exactly once before packaging.
- [x] Python is the single runtime-manifest/ZIP authority; dry-run package validation exercises a real temporary
      artifact without touching releases.
- [x] Python and JavaScript release authorities pass one shared SemVer corpus; installer tests prove exact
      stable/build/prerelease URLs without real network or system mutation.
- [x] `schema:check` is absent only under a contract that validates existing `plugin.json` development/release shapes
      and proves no additional schema/layout family exists; adding an unowned artifact fails `package:check`.
- [x] Full SemVer/prerelease/build classification is shared by local release and publisher; build metadata alone is not
      prerelease.
- [x] `publish-release.yml` is the sole workflow and contains only the exact allowlisted `publish-release` job and four
...
- [x] Publisher tag/artifact validation succeeds from a temporary checkout without `node_modules`; its JavaScript uses
      only Node built-ins and committed local modules.
- [x] No branch/PR CI, CODEOWNERS/ruleset, pre-commit, tag quality job, or remote release build is present or
      documented.
- [x] No real release artifact, commit, tag, push, or GitHub Release is produced during migration.

### Documentation and final validation

- [x] README, CONTRIBUTING, AGENTS/CLAUDE, quality/testing/smell/playbook/documentation-format/maintenance/release/plan
      authoring docs, and hook README match live commands and authority.
- [x] Required owner documentation lands in Phases 1-8 with the behavior it describes; Phase 9 changes only
      cross-document prose and leaves source, tools, package scripts, hooks, workflows, and release artifacts untouched.
- [x] `documentation/architecture/ui.md` names the extracted status owner and `ARCHITECTURE.md` maps the new
      type/test/policy roots.
- [x] CLAUDE is a checked short pointer to canonical AGENTS; obsolete sync tools are gone.
- [x] Documentation links include heading-fragment proof and no active stale command/system claims remain.
- [x] `npm run setup` and the complete clean/offline-capable validation sequence pass from a disposable clean
      environment.
- [x] Hook doctor proof, final test/coverage/inventory counts, package manifest count, `git diff --check`, and manual
      viewer smoke are recorded.
- [x] The final diff contains no unrelated user change.
- [x] PLAN5 is moved to completed only after all criteria pass.
