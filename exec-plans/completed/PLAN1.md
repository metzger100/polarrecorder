...
- Confidence indicator: curve segments are rendered from returned cells based on sample count per bin — cells below the display floor are not returned by `GET /api/polar`; lower-confidence returned cells use reduced visual emphasis; cells meeting the high-confidence floor use full emphasis. These thresholds are hardcoded display constants in `polar-chart.js` (not editable parameters — they are a rendering concern, not a data quality decision). For CSV, a default export includes cells once they meet the normal display sample floor, while `high_confidence=yes` leaves cells blank unless they meet `min_samples_for_export` (default 10). The diagram and CSV use related sample floors but live on different TWA grids — 1° vs. the preset's rows — so this is a floor correspondence, not a one-to-one cell correspondence.
...
- **"High-confidence cells only" checkbox:** unchecked by default. Label: "High-confidence cells only (≥ N samples)", where N is the live `min_samples_for_export` config value (read from the cached `GET /api/config`, fetched once on Export-tab activation per §6.G.8; fall back to "10" if unavailable). A short helper line uses CSV-specific wording: "Off: export cells once they meet the normal display sample floor. On: leave cells blank unless they meet the stricter high-confidence sample floor." When unchecked, the download/preview calls omit `high_confidence` (server defaults to the `MIN_SAMPLES_DISPLAY` floor, so the export matches the preview exactly). When checked, the calls append `&high_confidence=yes` (server uses the `min_samples_for_export` floor). The checkbox affects only cell population — not the grid, format, or filename.
...
JSON backup, disabled Post-MVP restore, and destructive reset live in the Settings tab, not the Export tab.
...
**Testing approach for UI:** Manual visual testing in a browser using `tools/mock-server.py` — a minimal stdlib-only Python HTTP server that serves static viewer files and deterministic in-memory API responses seeded from representative mock data. Launch: `python tools/mock-server.py` (serves at `http://localhost:8080`). Open `viewer.html` with `<body data-api-base="http://localhost:8080/api/">` (or pass it as a query param). The mock server also serves `viewer.html` and all static files from the plugin directory so CORS is not an issue. Mock data should contain representative data: multiple TWS bands, a mix of accepted/rejected timeline entries, non-zero counters, and mutable preset/pause/reset behavior for manual review. Vitest + jsdom tests for data transformation logic (export grid projection, histogram percentile extraction) if those functions are factored into testable JS modules loaded during testing.
...
- `documentation/` skeleton: `TABLEOFCONTENTS.md`, `core-principles.md`, all convention and guide files listed in Section 7. Any file that is a stub at end of Phase 1 (only those §12 marks with a stub→complete lifecycle — currently `README.md`) must still carry all four required sections (Status/Overview/Key Details/Related) so the now-blocking `check-doc-format` stays green; every other documentation file introduced in Phase 1 is written complete (see the §10 per-phase completeness invariant).
...
- `export-ui.js` — Export configurator: preset selector, editable TWA/TWS grid (individual number inputs with add/remove, auto-sort, inline validation), percentile override, save-as-preset with overwrite confirmation, CSV preview, and CSV download trigger.
...
- `tools/mock-server.py` — Minimal stdlib-only Python HTTP server for UI testing. Serves at `http://localhost:8080`. No external dependencies. It serves static files from the project root directory and deterministic in-memory API responses generated from one mock polar dataset. Query parameters affect `polar`, `export`, and preset mutations. Pause/resume changes later `status.recording`; preset save/delete changes later `presets`, `polar?format=<name>`, and `export?format=<name>` responses; confirmed reset changes later model/export/status/backup responses to show an empty learned model. State is process-local only, with no persistence or database. This mirrors the production URL structure where `viewer.html` and API endpoints are siblings under `/plugins/polarrecorder/`. CORS headers are not needed because both static files and API responses are served from the same origin.
- `tests/mock-data/status.json`, `polar.json`, `rejections.json`, `timeline.json`, `config.json`, `presets.json`, `export-windy.csv`, `export-json.json` — Representative initial fixtures for manual browser review and API-shape inspection. Must include: multiple TWS bands with data, a `timeline.json` with a mix of accepted-dominant, rejected-dominant (e.g. an anchored band with `reject_anchored`), and mixed 1-minute buckets plus at least one gap minute, non-zero counters, realistic current_values and a `current_decision` (so the recent-decisions strip renders during manual testing), a valid Windy-format CSV export, a valid full-model JSON export, and a presets response with Windy built-in plus at least one user-saved preset.
...
- Tab navigation works: 5 tabs (Polar, Status, Timeline, Export, Settings), switching shows/hides content.
...
- [ ] Tab navigation works across all 5 tabs (Polar, Status, Timeline, Export, Settings).
